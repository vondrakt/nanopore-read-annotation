...
# This script will take the pseudocoded multifasta file as an input and create a tabular output.
# This tabular output will have rows corresponding to different satellites and columns
# corresponding to the windows left and right of a satellite array which will count the occurence
# of other satellites within the window.
# This output data will then be plotted by an R script.
...
with open(options.coding) as c:
    ...
    for line in c:
        # dividing columns of each line into separate items
        items = line.split()
...
def complement(read, dictionary):
    ...
    while i > -len(read) - 1:
        ...
        # decrease i by 1
        i -= 1
    # join the complement_read list into a string
...
def left_neighbour(pseudo, working_read, left):
    ...
    for i in first_positions:
        ...
        # calculate the starting position of the window
        ...
        # iterating over the window sequence (keep in mind to start from the right side to the left)
        i = -1
...
with open(options.coding) as c:
    ...
    for line in c:
        # divide the columns of each line into separate items
        items = line.split()
