...
# This script takes a pseudocoded multifasta file as an input and creates
# a tabular output which takes form:
...
with open(options.coding_table) as c:
    ...
    for line in c:
        # split columns of each line into separate items
        items = line.split()
    ...
    for idx, n in enumerate(val):
        ...
        if length_data[-1][4] == n:
                ...
                if idx - length_data[-1][2] == 1 or idx - length_data[-1][2] == 0:
                    ...
                    length_data[-1][2] = idx
                # if the array is not continuous
                else:
...
# iterating over the length_data
for i in length_data:
    ...
    if i[1] < int(options.start_pos) or i[2] > (read_length - int(options.start_pos)):
        ...
        line = str(i[3])+'\t'+str(length)+'\t'+str(i[4])+'\t'+str(read_length)+'\t'+'T'+'\n'
    else:
        # if it does not then the array is intact
        # create the correct output format and write out line
        line = str(i[3])+'\t'+str(length)+'\t'+str(i[4])+'\t'+str(read_length)+'\t'+'I'+'\n'
